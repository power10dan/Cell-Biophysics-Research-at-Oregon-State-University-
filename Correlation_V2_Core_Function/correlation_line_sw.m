...
for thetaind = 1:length(angles)                    
    for bind = 1:length(trans)      
        ...
        %binning, or rearranging data into smaller, manageable chuncks to
        %reduce observation errors
        temp2 = reshape(temp,[subimgsz(1),imgsize(1)/subimgsz(1),...
    ...
    waitbar(thetaind/length(angles), h,'Processing'); 
end
...
colsubimgs = reshape(sgtestimage,...
 ...
 waitbar(h,'Analysis Complete, cleaning up');
 pause(0.6); % don't dismiss waitbar too fast
