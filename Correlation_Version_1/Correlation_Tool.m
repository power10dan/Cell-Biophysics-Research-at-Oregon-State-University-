...
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    switch(user_response)
        ...
        case 'Folder Input'
            ...
            direct_choice = inputdlg('Please specify the path to your desired directory, or leave the field empty and press ok to manually find your directory', 'Dialog', [1 100]);
            
            if isempty(direct_choice)
                
               return;
                
            end 
            
            user_input = direct_choice{:};                       
            path = uigetdir(user_input);
            ...
            folder = dir(path); % get everything from current directory 
            ind = [folder(:).isdir]; % find all directories
            nodir = find(~ind); % find the indices of non-directories
           
            newFolder = folder(nodir);% store the non-directories into a new variable
            [new_data_joined,name_list] = join_text(newFolder, handles.listbox1);
           
            name_list_full = new_data_joined;
            name_list_added_element = name_list;
            
           
            if isempty(path_storage)
               
                path_storage = cell(1, numel(name_list_full));
                file_storage = cell(1,numel(name_list_full));
                string_file_storage = cell(1, numel(name_list_full));
                
            end
           
            for idx = 1:numel(name_list_added_element)
              
                file_storage{idx} = strcat(path, filesep, name_list_added_element{idx});
                
                string_file_storage{idx}= file_storage{idx};
              
            end
     
            empty_cell = cellfun(@isempty, path_storage);
            
            empty_index = find(empty_cell);
          
            if numel(empty_index) > 0
           
                for idx = 1:numel(empty_index)
                
                    path_storage{empty_index(idx)} = string_file_storage{idx};
                ...
                empty_index = find(cellfun(@isempty, path_storage)); % find the newly created empty cells
                
                for idx = 1:numel(name_list_added_element)
                    
                    path_storage{empty_index(idx)} = string_file_storage{idx}; %put the new data in
            ...
            set(handles.listbox1, 'string', name_list_full);
            set(handles.listbox1, 'string', name_list_full, 'Value',1);
...
function pushbutton2_Callback(hObject, eventdata, handles)
    ...
    if numel(index_selected) > 1
        ...
        for indx = index_selected(1):index_selected(numel(index_selected)) %start from 1st item selected to last item selected          
